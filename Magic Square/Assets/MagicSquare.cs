...
public class MagicSquare : MonoBehaviour {
    ...
    void ButtonPress (int Button) {
        ...
        if (hasSelected) {
            ...
            if (CheckIfCorrect()) {
                if (MagicList.SequenceEqual(IntendedSolution))
                    Debug.LogFormat("[Magic Square #{0}] Solved with the intended solution.", ModuleId);
                else
                    {
                        Debug.LogFormat("[Magic Square #{0}] Solved with the following solution:", ModuleId);
                        Debug.LogFormat("[Magic Square #{0}] {1} {2} {3}", ModuleId, MagicList[0].ToString("00"), MagicList[1].ToString("00"), MagicList[2].ToString("00"));
                        Debug.LogFormat("[Magic Square #{0}] {1} {2} {3}", ModuleId, MagicList[3].ToString("00"), MagicList[4].ToString("00"), MagicList[5].ToString("00"));
                        Debug.LogFormat("[Magic Square #{0}] {1} {2} {3}", ModuleId, MagicList[6].ToString("00"), MagicList[7].ToString("00"), MagicList[8].ToString("00"));
                    }
    ...
    bool CheckIfCorrect () {
        int[] Sum = new int[8];
        for (int i = 0; i < 9; i++) { // Sum of each row.
            Sum[i / 3] += MagicList[i];
        }
        for (int i = 0; i < 9; i++) { // Sum of each column.
            Sum[i % 3 + 3] += MagicList[i];
        }
        // Sum of main + alt diagonal.
        Sum[6] = MagicList[0] + MagicList[4] + MagicList[8];
        Sum[7] = MagicList[2] + MagicList[4] + MagicList[6];

        return Sum.Distinct().Count() == 1;
    }
...
#pragma warning restore 414
    ...
    private IEnumerator ProcessTwitchCommand (string command) {
        ...
        var ixs = new int[2];
        if (!(m.Success && int.TryParse(m.Groups[1].Value, out ixs[0]) && int.TryParse(m.Groups[2].Value, out ixs[1]) && MagicList.Contains(ixs[0]) && MagicList.Contains(ixs[1])))
        {
            yield return "sendtochaterror Invalid command! Use \"!{1} swap ## ##\" to swap the two tiles.";
            yield break;
        }
        yield return null;
